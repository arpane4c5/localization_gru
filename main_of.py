...
@Description: Train an RNN on OF features for cricket stroke localization 
...
OFFeatsPath = "/home/arpan/VisionWorkspace/Cricket/localization_rnn/OF_npy_grid"+str(grid)
OFMainFeatsPath = "/home/arpan/VisionWorkspace/Cricket/localization_gru/OF_npy_mainDataset_train_grid"+str(grid)
OFValFeatsPath = "/home/arpan/VisionWorkspace/Cricket/localization_gru/OF_npy_mainDataset_test_grid"+str(grid)
base_name = "/home/arpan/VisionWorkspace/Cricket/localization_gru/logs/GRU_of"+str(grid)+"HLMain_log_hidden1k"
...
if os.path.exists("/opt/datasets/cricket/ICC_WT20"):
    ...
    OFFeatsPath = "/home/arpan/DATA_Drive/Cricket/extracted_feats/OF_npy_grid"+str(grid)
    OFMainFeatsPath = "/home/arpan/DATA_Drive/Cricket/extracted_feats/OF_npy_mainDataset_train_grid"+str(grid)
    OFValFeatsPath = "/home/arpan/DATA_Drive/Cricket/extracted_feats/OF_npy_mainDataset_test_grid"+str(grid)
    base_name = "/home/arpan/VisionWorkspace/localization_gru/logs/GRU_of"+str(grid)+"HLMain_log_hidden1k"
...
#                elif phase == 'test':
#                    batchFeats = utils.getBatchFeatures(testFeats, keys, seqs, motion=True)
                ...
                # num_ft_vecs sent to RNN = SEQ_SIZE - (#frames in OF - 1) 
                #accuracy += get_accuracy(preds, y, (SEQ_SIZE - (c3dWinSize-1)))
                accuracy += get_accuracy(preds, y, SEQ_SIZE-(2-1))
...
#                      training_stats[epoch]['test']['acc']))
    ...
    "losses_OF_ep"+str(epoch+1)+"_seq"+str(SEQ_SIZE)+"_Adam.pkl")
...
def save_model_checkpoint(base_name, model, ep, opt, win=16, use_gpu=True):
    ...
    name = os.path.join(base_name, "GRU_OF_ep"+str(ep)+"_seq"+str(win)+"_"+opt+".pt")
...
def main(base_name, SEQ_SIZE=16, BATCH_SIZE=256, HIDDEN_SIZE=1000, N_EPOCHS=30, \
    ...
    features = utils.readAllPartitionFeatures(OFFeatsPath, train_lst)
    mainFeatures = utils.readAllPartitionFeatures(OFMainFeatsPath, train_lst_main)
...
#    # Training the model on the features for N_EPOCHS 
    ...
    mod_file = os.path.join(base_name, \
                "GRU_OF_ep"+str(N_EPOCHS)+"_seq"+str(SEQ_SIZE)+"_Adam.pt")
    ...
    classifier.eval()
    val_keys, predictions = predict(OFValFeatsPath, val_lst_main, classifier, val_loader, \
                                    use_gpu)
...
if __name__=='__main__':
    ...
    description = "Script for training RNNs on OF Grid features"
