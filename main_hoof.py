...
# -*- coding: utf-8 -*-
"""
Created on Sat June 27 12:53:27 2019

@author: Arpan

@Description: Train an RNN on HOG features for cricket stroke localization 
...
HOOFFeatsPath = "/home/arpan/VisionWorkspace/Cricket/localization_gru/HOOF_HL_bins"+str(nbins)+"_th"+str(mag_thresh)
HOOFMainFeatsPath = "/home/arpan/VisionWorkspace/Cricket/localization_gru/HOOF_mainDataset_train_bins"+str(nbins)+"_th"+str(mag_thresh)
HOOFValFeatsPath = "/home/arpan/VisionWorkspace/Cricket/localization_gru/HOOF_mainDataset_test_bins"+str(nbins)+"_th"+str(mag_thresh)
base_name = "/home/arpan/VisionWorkspace/Cricket/localization_gru/logs/GRU_hoofHLMain_log_hidden1k"
...
if os.path.exists("/opt/datasets/cricket/ICC_WT20"):
    ...
    HOOFFeatsPath = "/home/arpan/DATA_Drive/Cricket/extracted_feats/HOOF_HL_bins"+str(nbins)+"_th"+str(mag_thresh)
    HOOFMainFeatsPath = "/home/arpan/DATA_Drive/Cricket/extracted_feats/HOOF_mainDataset_train_bins"+str(nbins)+"_th"+str(mag_thresh)
    HOOFValFeatsPath = "/home/arpan/DATA_Drive/Cricket/extracted_feats/HOOF_mainDataset_test_bins"+str(nbins)+"_th"+str(mag_thresh)
    base_name = "/home/arpan/VisionWorkspace/localization_gru/logs/GRU_hoofHLMain_log_hidden1k"
    ...
    for epoch in range(nEpochs):
        ...
        for phase in ['train']:
            ...
            for i, (keys, seqs, labels) in enumerate(dataset):
                ...
                if phase == 'train':
                    batchFeats = utils.getBatchFeatures(trainFeats, keys, seqs, motion=True)
#                elif phase == 'test':
#                    batchFeats = utils.getBatchFeatures(testFeats, keys, seqs, motion=False)
                
                # return the torch.Tensor values for inputs and 
                x, y = utils.make_variables_new(batchFeats, labels, motion=True, use_gpu=use_gpu)
                ...
                accuracy += get_accuracy(preds, y, SEQ_SIZE-1)
...
#                      training_stats[epoch]['test']['acc']))
    ...
    "losses_HOOF_ep"+str(epoch+1)+"_seq"+str(SEQ_SIZE)+"_Adam.pkl")
    with open(loss_filename, 'wb') as fr:
        pickle.dump(training_stats, fr, protocol=pickle.HIGHEST_PROTOCOL)
    # Training finished


def predict(featuresPath, val_lst, classifier, val_loader, use_gpu):
    ...
    for i, (keys, seqs, labels) in enumerate(val_loader):
        ...
        #break
        # Validation stage
        inputs, target = utils.make_variables_new(batchFeats, labels, motion=True, use_gpu=use_gpu)
...
def save_model_checkpoint(base_name, model, ep, opt, win=16, use_gpu=True):
    ...
    name = os.path.join(base_name, "GRU_HOOF_ep"+str(ep)+"_seq"+str(win)+"_"+opt+".pt")
...
def main(base_name, SEQ_SIZE=16, BATCH_SIZE=256, HIDDEN_SIZE=1000, N_EPOCHS=30, \
    ...
    features = utils.readAllPartitionFeatures(HOOFFeatsPath, train_lst)
    mainFeatures = utils.readAllPartitionFeatures(HOOFMainFeatsPath, train_lst_main)
...
#    # Training the model on the features for N_EPOCHS 
    ...
    mod_file = os.path.join(base_name, \
                "GRU_HOOF_ep"+str(N_EPOCHS)+"_seq"+str(SEQ_SIZE)+"_Adam.pt")
    ...
    classifier.eval()
    val_keys, predictions = predict(HOOFValFeatsPath, val_lst_main, classifier, val_loader, \
                                    use_gpu)
...
#    print localization_dict
    ...
    filt_shots_filename = os.path.join(base_name, "predicted_localizations_test_th0_5_filt"\
...
if __name__=='__main__':
    ...
    description = "Script for training RNNs on HOOF features"
