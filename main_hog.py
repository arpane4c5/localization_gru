...
HOGFeatsPath = "/home/arpan/VisionWorkspace/Cricket/localization_rnn/hog_feats_64x64"
HOGMainFeatsPath = "/home/arpan/VisionWorkspace/Cricket/hog_feats_64x64_mainDataset_train"
HOGValFeatsPath = "/home/arpan/VisionWorkspace/Cricket/hog_feats_64x64_mainDataset_train"
base_name = "/home/arpan/VisionWorkspace/Cricket/localization_gru/logs/GRU_hogHLMain_log_hidden1k"
...
if os.path.exists("/opt/datasets/cricket/ICC_WT20"):
    ...
    HOGFeatsPath = "/home/arpan/VisionWorkspace/localization_rnn/hog_feats_64x64"
    HOGMainFeatsPath = "/home/arpan/DATA_Drive/Cricket/extracted_feats/hog_feats_64x64_mainDataset_train"
    HOGValFeatsPath = "/home/arpan/DATA_Drive/Cricket/extracted_feats/hog_feats_64x64_mainDataset_test"
    base_name = "/home/arpan/VisionWorkspace/localization_gru/logs/GRU_hogHLMain_log_hidden1k"
    ...
    for epoch in range(nEpochs):
        ...
        for phase in ['train']:
            ...
            for i, (keys, seqs, labels) in enumerate(dataset):
                ...
                if phase == 'train':
                    batchFeats = utils.getBatchFeatures(trainFeats, keys, seqs, motion=False)
#                elif phase == 'test':
#                    batchFeats = utils.getBatchFeatures(testFeats, keys, seqs, motion=False)
                
                # return the torch.Tensor values for inputs and 
                x, y = utils.make_variables_new(batchFeats, labels, motion=False, use_gpu=use_gpu)
                ...
                accuracy += get_accuracy(preds, y, SEQ_SIZE)
...
#                      training_stats[epoch]['test']['acc']))
    ...
    "losses_HOG_ep"+str(epoch+1)+"_seq"+str(SEQ_SIZE)+"_Adam.pkl")
...
def predict(featuresPath, val_lst, classifier, val_loader, use_gpu):
    ...
    for i, (keys, seqs, labels) in enumerate(val_loader):
        ...
        #break
        # Validation stage
        inputs, target = utils.make_variables_new(batchFeats, labels, motion=False, use_gpu=use_gpu)
...
def save_model_checkpoint(base_name, model, ep, opt, win=16, use_gpu=True):
    ...
    name = os.path.join(base_name, "GRU_HOG_ep"+str(ep)+"_seq"+str(win)+"_"+opt+".pt")
...
if __name__=='__main__':
    ...
    description = "Script for training RNNs on HOG features"
